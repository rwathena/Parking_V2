...
public class SysAdminActivity extends BaseActivity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.btn_serach:
			if (search.getText().toString().equals("")) {// TODO check phoneNum
				Toast.makeText(SysAdminActivity.this, "号码不符合规则",
		...
		case R.id.btn_update:
			final ConfirmDialog confirmDialog = new ConfirmDialog(
					SysAdminActivity.this, "确定要跟新信息吗?", "确定", "取消");
			confirmDialog.show();
			confirmDialog
					.setClicklistener(new ConfirmDialog.ClickListenerInterface() {
						...
						public void doConfirm() {
							...
							if (!etUserPhone.getText().toString().trim()
									.equals("")) {
								user.setPhoneNum(etUserPhone.getText()
										.toString());
							} else {
							...
							if (!etUserIdNum.getText().toString().trim()
									.equals("")) {

								user.setIdentityNum(etUserIdNum.getText()
										.toString());
							} else {
								user.setIdentityNum(user.getIdentityNum());
							}
		...
		case R.id.btn_signout:
			// 删除用户名和密码记录
			SharedPreferences prefNoVersion = getApplicationContext()
					.getSharedPreferences(ClientService.PREF_NAME, 0);
	...
	private void updateListViewCars() {
		listViewCars.setAdapter(new MyCarsAdeptet(SysAdminActivity.this,
				userCars));
		listViewCars.setOnItemClickListener(new OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> adapterView, View v, int i,
					long arg3) {
				final CarInfo car = (CarInfo) adapterView.getAdapter().getItem(
						i);
				// TODO new dialog
				if (car != null) {
					Log.i("num: " + car.getCarNum());
					final ConfirmDialog confirmDialog = new ConfirmDialog(
							SysAdminActivity.this, "确定该车要开始业务吗?", "确定", "取消");
					confirmDialog.show();
					confirmDialog
							.setClicklistener(new ConfirmDialog.ClickListenerInterface() {
								...
								public void doConfirm() {
									confirmDialog.dismiss();
									car.setCarState(CarInfo.CAR_STATE_ACTIVE_VALUE);
									ClientServerMessage m = new ClientServerMessage();
					...
					// new BaseDialog(mainActivity,
					// R.layout.device_list).show();
